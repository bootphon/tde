...
if __name__ == '__main__':
    ...
    def parse_args():
        parser = argparse.ArgumentParser(
            ...
            description='Evaluate spoken term discovery on the mandarin dataset',
            epilog="""Example usage:

$ ./mandarin_eval2 my_sample.classes resultsdir/

evaluates STD output `my_sample.classes` on the mandarin dataset and stores the
...
""")
    ...
    fragments_cross_file  = path.join(resource_dir, 'mandarin.intervals.cross')
    fragments_within_file = path.join(resource_dir, 'mandarin.intervals.within')
    ...
    split_file            = path.join(resource_dir, 'mandarin.split')

    if verbose:
        print 'mandarin_eval2 version {0}'.format(VERSION)
        print '---------------------------'
        print 'dataset:     mandarin'
