...
if __name__ == '__main__':
    ...
    def parse_args():
        parser = argparse.ArgumentParser(
            ...
            description='Evaluate spoken term discovery on the french dataset',
            epilog="""Example usage:

$ ./french_eval2 my_sample.classes resultsdir/

evaluates STD output `my_sample.classes` on the french dataset and stores the
...
""")
    ...
    fragments_cross_file  = path.join(resource_dir, 'french.intervals.cross')
    fragments_within_file = path.join(resource_dir, 'french.intervals.within')
    ...
    split_file            = path.join(resource_dir, 'french.split')

    if verbose:
        print 'french_eval2 version {0}'.format(VERSION)
        print '---------------------------'
        print 'dataset:     french'
