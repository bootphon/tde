...
def evaluate_group(disc_clsdict, verbose=False, debug=False):
    ...
    if len(pclus_nmatch) == 0:
        ...
        prec = sum(len(t) * ws_disc[t] * pclus_pgoldclus_nmatch[t] / pclus_nmatch[t]
                   for t in ts_disc if pclus_nmatch[t] > 0) / len_disc
    ...
    if len(pgoldclus_nmatch) == 0:
        ...
        rec = sum(len(t) * ws_gold[t] * pclus_pgoldclus_nmatch[t] / pgoldclus_nmatch[t]
                  for t in ts_gold if pgoldclus_nmatch[t] > 0) / len_gold
