...
def eval_from_psets(pdisc, pgold, psubs, verbose=False, debug=False):
    ...
    ts_gold = make_typeset(pgold, verbose, debug)  # pgold already contains substring completion
    ...
    # ts_disc = [t for t in ts_disc if psubs_nmatch[t] > 0]  # most likely useless
    # ts_gold = [t for t in ts_gold if pgold_nmatch[t] > 0]
    ...
    if len(psubs_nmatch) == 0:
        ...
        prec = sum(len(t) * psubs_pgold_nmatch[t] / psubs_nmatch[t]
                   for t in ts_disc) / (len_disc * len(ts_disc))
    ...
    if len(pgold_nmatch) == 0:
        ...
        rec = sum(len(t) * psubs_pgold_nmatch[t] / pgold_nmatch[t]
                  for t in ts_disc) / (len_gold * len(ts_gold))
